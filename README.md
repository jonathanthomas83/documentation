# Table of contents

## GOV.UK Prototype Kit

- [Manually deploying to Heroku using command line](/prototype-kit/deploying-to-heroku-command-line.md)
- [Pass data from page to page](/prototype-kit/pass-data-from-page-to-page.md)
- [Today's date and date manipulation using 'Moment'](/prototype-kit/todays-date-using-moment.md)
- [Show alternative placeholder text when session data is unavailable](/prototype-kit/show-alternative-placeholder-text-when-session-data-is-unavailable.md)

## HUGO

- [Deploying HUGO with GitHub Actions](/hugo/hugo-github-actions-deployment.md)

## Command line hacks

- [grep](command-line/grep.md)
- [Find files](command-line/find-files.md)

## Useful things

- [Portable Filenames (The POSIX standard)](portable_filenames_POSIX.md)

## Helpful links

- [Helpful links](helpful_links.md)
